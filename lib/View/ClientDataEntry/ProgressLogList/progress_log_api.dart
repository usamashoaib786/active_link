...
class ProgressApiServices {
  Future<File?> progressLogs(
      {required AppDio dio, required BuildContext context}) async {
    try {
      final response = await dio.get(path: AppUrls.progressLogList);
          ...
          pw.Page(
            build: (pw.Context context) => pw.Table.fromTextArray(
              ...
              data: List<List<String>>.generate(
                ...
                (index) {
                  final behavior = response.data['data'][index];
                  final name = response.data["staffdetails"][0]["full_name"];
                  return [
                    behavior['client_pogress_id'].toString(),
                    "${behavior['f_name']} ${behavior['l_name']}",
                    "$name",
                    behavior['pogress_created'].toString(),
                  ];
                },
              ),
            ),
